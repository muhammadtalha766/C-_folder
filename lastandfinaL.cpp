...
int main() {
    ...
    for (int i = 1; i <= numFloors; i++) {
        ...
        if (numFloors != 2) { // Assuming garages are not present on the 2nd floor
            ...
            for (int j = 1; j <= numGarages; j++) {
                ...
                garageCost += garageArea * 50000.0; // assume PKR 50000 per square meter for garage
    ...
    double netArea = constableArea - (totalWindowArea + totalDoorArea);
    ...
    cout << "Net area (constable area minus window and door area): " << netArea << " square meters" << endl;
